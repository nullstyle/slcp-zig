//! Expected-fail case for `zig build owned-appnode-errors`: no `pub fn initState`.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): missing `pub fn initState(context: Context, gpa: std.mem.Allocator) InitError!State`.
