//! Expected-fail case for `zig build owned-appnode-errors`: no `pub fn validate`.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): missing `pub fn validate(state: *const State, cmd: Command, context: slcp.ValueContext) slcp.Validity`.
