//! Expected-fail case for `zig build owned-appnode-errors`: no `pub fn observe`.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): missing `pub fn observe(state: *const State, gpa: std.mem.Allocator) std.mem.Allocator.Error!Obs`.
