//! Expected-fail case for `zig build owned-appnode-errors`: Obs contains a slice but deinitObs is absent.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): Obs owns memory (it contains a pointer) but has no deinitObs.
...
const Bad = struct {
    ...
    pub const Obs = struct { entries: []const u64 };
