//! Expected-fail case for `zig build owned-appnode-errors`: no `pub const Context`.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): missing `pub const Context` — the startup context type.
...
const Bad = struct {
    ...
    pub fn initState(context: void, gpa: std.mem.Allocator) InitError!State {
        _ = context;
        _ = gpa;
        return .{};
    }
