//! Expected-fail case for `zig build owned-appnode-errors`: no `pub const Obs`.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): missing `pub const Obs` — the per-slot observation type.
...
const Bad = struct {
    ...
    pub fn observe(state: *const State, gpa: std.mem.Allocator) std.mem.Allocator.Error!u64 {
        _ = gpa;
        return state.count;
    }
};
