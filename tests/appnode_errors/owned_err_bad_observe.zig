//! Expected-fail case for `zig build owned-appnode-errors`: observe takes no allocator and cannot fail.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): observe has the wrong signature.
...
const Bad = struct {
    ...
    pub fn observe(state: *const State) Obs {
