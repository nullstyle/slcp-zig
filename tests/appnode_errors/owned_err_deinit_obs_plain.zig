//! Expected-fail case for `zig build owned-appnode-errors`: Obs is plain data yet deinitObs is declared.
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): deinitObs is declared, but Obs is plain by-value data — there is nothing to free.
