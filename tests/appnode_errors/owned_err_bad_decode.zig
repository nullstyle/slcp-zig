//! Expected-fail case for `zig build owned-appnode-errors`: decode returns the wrong type.
...
const Bad = struct {
    ...
    pub fn encode(cmd: Command, buf: []u8) []u8 {
        std.mem.writeInt(u64, buf[0..8], cmd.next, .big);
        return buf[0..8];
    }
    pub fn decode(bytes: []const u8) ?u64 {
