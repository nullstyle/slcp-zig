//! Expected-fail case for `zig build owned-appnode-errors`: no `pub const InitError` (initState carries an explicit set so it still analyzes).
//! Pinned needle (tail of the first error line, build.zig owned_appnode_error_cases):
//!   ): missing `pub const InitError` — the explicit error set initState returns.
...
const Bad = struct {
    ...
    pub fn initState(context: Context, gpa: std.mem.Allocator) error{OutOfMemory}!State {
        _ = context;
        _ = gpa;
        return .{};
    }
